...
public:
    ...
    std::vector<Eigen::Vector2d> findPath(Eigen::Vector2d start, Eigen::Vector2d goal) {
        ...
        if (grid_map_.grid[gridStart.first][gridStart.second] == 1 ||
            ...
            ROS_ERROR("Start or Goal is inside an obstacle, please re-launch the program!");
            return {};
        }

        
        std::set<std::shared_ptr<Node>,cmp> open_list; 
        ...
        while (!open_list.empty()) {
            ...
            for (const auto& neighbor : neighbors) {
                ...
                auto it = std::find_if(open_list.begin(), open_list.end(), [&](const std::shared_ptr<Node>& n) {
                    return n->x == neighbor.x && n->y == neighbor.y;
                });

                if (it != open_list.end()) {
                    if ((*it)->g_cost > new_g_cost) {
                        open_list.erase(it); 
...
int main(int argc, char** argv) {
    ...
    setlocale(LC_ALL,"");
    ros::init(argc, argv, "astar_planner_2");
    ros::NodeHandle nh;
    double map_min_, map_max_, grid_resolution_;
    ...
    ros::Rate rate(10);
    ros::Publisher path_pub = nh.advertise<nav_msgs::Path>("path_2", 1);
    ...
    while (ros::ok()) {
        ...
        ROS_INFO("current findPath iteration finished! Search time=%.5f for A-star using std::set",search_time_duration.toSec());
